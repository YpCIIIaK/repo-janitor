...
async function scanJsTsExports(ctx: ScanContext, sources: string[]): Promise<Issue[]> {
    ...
    // Exports referenced within their OWN module. An export used
    // locally isn't dead — removing it would break the file — even when nothing
    // imports it; flagging those is a false positive (e.g. a helper that's exported
    // for tests but also called in-module). Keyed by file so identical names in
    // different modules don't cross-suppress.
    const locallyUsed = new Set<string>()
    const localKey = (file: string, name: string) => `${file}\u0000${name}`
    ...
    for (const file of sources) {
      ...
      // Count every identifier occurrence in this file. A name that appears more
      // than once (its declaration plus at least one reference) is used somewhere
      // in the module, so an export of that name isn't dead even if un-imported.
      ...
      for (const [name, count] of idCounts) {
        if (count > 1) locallyUsed.add(localKey(file, name))
      }
    }
    ...
    for (const exp of exports) {
      ...
      if (kit) {
        // Defer to the per-kit summary instead of one issue per primitive. The kit
        // summary is counted before the in-module check below, since UI primitives
        // routinely self-reference (e.g. `Foo.displayName = "Foo"`) yet are still
        // genuinely unused when nothing imports them.
      ...
      // Used as a value elsewhere in its own module → not dead, even if un-imported.
      if (locallyUsed.has(localKey(exp.file, exp.name))) continue
