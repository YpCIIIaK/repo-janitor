 ...
 *
 * Shared by both report routes: `/r/<owner>/<name>/<token>`, which reads a share
 * someone published from their browser, and `/r/<owner>/<name>`, which reads the
 * latest report the repository's own CI uploaded. The two differ in where the
 * report comes from and in whether a token is required — not in a single pixel
 * of what a reader sees, so they must not be two copies of this markup.
 *
 ...
 *
 * Outside the component on purpose: these pages are `force-dynamic`, so reading
 * the clock per request is the intent, but doing it inline makes the component
