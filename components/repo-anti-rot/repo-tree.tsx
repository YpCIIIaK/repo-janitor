...
import { buildFileTree, countNodes, type TreeNode, type TreeNodeKind } from "@/lib/file-tree"
...
function RotNode({ data }: NodeProps<Node<RotNodeData>>) {
  const { tree, orientation, collapsible, collapsed, selected } = data
...
export function RepoTree({
  ...
  const root = useMemo(
    () => buildFileTree(issues, weights, { rootLabel: `${repo.owner}/${repo.name}` }),
    [issues, weights, repo.owner, repo.name],
  )
  ...
  const { nodes, edges } = useMemo(
    () => layout(root, collapsed, orientation, selectedId),
    [root, collapsed, orientation, selectedId],
  )
  ...
  const onNodeClick = useCallback(
    (_: unknown, rfNode: Node) => {
      ...
      if (!tn) return
      // Directories/buckets with children act as expand/collapse toggles; leaf
      // files (and empty buckets) open the detail card.
  ...
  if (root.children.length === 0) {
    return (
      <div className="flex h-[480px] flex-col items-center justify-center rounded-lg border border-dashed border-border text-center">
        <Boxes className="size-8 text-muted-foreground/50" />
        <p className="mt-3 text-sm font-medium">Nothing to map</p>
        <p className="mt-1 max-w-xs text-xs text-muted-foreground">
          No findings resolved to a file or branch — the last scan was clean.
        </p>
      </div>
    )
  }
      ...
      <div className="pointer-events-none absolute bottom-3 left-1/2 z-10 flex -translate-x-1/2 items-center gap-3 rounded-full border border-border bg-card/90 px-3 py-1 text-[10px] text-muted-foreground shadow-sm backdrop-blur">
        {(["critical", "warning", "info"] as const).map((sev) => (
          <span key={sev} className="flex items-center gap-1">
            <span className={cn("size-2 rounded-full", ACCENT[sev].replace("border-l-", "bg-"))} />
            {severityLabels[sev]}
          </span>
        ))}
      </div>
